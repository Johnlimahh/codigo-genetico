...
function finalizeSpin() {
    ...
    // Gera um novo macaco baseado no selecionado, mas respeitando o macaco de referência
    ...
    if (!isDuplicate) {
        ...
        setTimeout(() => animateRoulette(1000), 200);
        return;
    }

    if (selectedCount < populationSize) {
        // Habilita o botão para girar novamente
        document.getElementById("spinBtn").disabled = false;
    } else {
...
window.onload = function() {
    ...
    // Iniciar com a primeira população
    generatePopulation();
};
